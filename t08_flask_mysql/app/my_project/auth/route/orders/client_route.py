...
def get_all_clients() -> Response:
    ...
    return make_response(jsonify(clients), HTTPStatus.OK)
...
def get_client(client_id: int) -> Response:
    ...
    if client:
        return make_response(jsonify(client), HTTPStatus.OK)
    return make_response(jsonify({"error": "Client not found"}), HTTPStatus.NOT_FOUND)
